...
async function loadBinTypes() {
  const { data, error } = await db.from('storage_bin_types').select('*').order('name', { ascending: true });
...
function populateBinTypeSelects() {
  ...
  if (locSel) locSel.innerHTML = `<option value="">All Locations</option>` + locs.map(l=>`<option value="${l}">${escapeHtml(l)}</option>`).join('');
  const typFil = document.getElementById('filter-type');
  if (typFil) typFil.innerHTML = `<option value="">All Types</option>` + allBinTypes.map(bt=>`<option value="${bt.id}">${escapeHtml(bt.name)}</option>`).join('');
}
...
function renderBinTypeList() {
  ...
  if (allBinTypes.length === 0) { list.innerHTML = `<div class="empty-state" style="padding:24px;"><p>No bin types yet.</p></div>`; return; }
  list.innerHTML = allBinTypes.map(bt => `
    <div class="bin-type-item">
      <div class="bin-type-photo">${bt.photo_url?`<img src="${bt.photo_url}" alt="">`:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="20" height="20"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>`}</div>
      <span class="bin-type-name">${escapeHtml(bt.name)}</span>
      <button class="bin-type-delete" onclick="deleteBinType('${bt.id}')" title="Delete"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="16" height="16"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg></button>
    </div>`).join('');
}

async function loadContainers() {
  const { data, error } = await db.from('storage_containers').select('*, storage_bin_types(name)').order('code', { ascending: true });
  if (error) { console.error(error); document.getElementById('container-list').innerHTML=`<div class="empty-state"><p>Error loading inventory.</p></div>`; return; }
  allContainers = data || [];
  const ids = allContainers.map(c => c.id);
  if (ids.length > 0) {
    const { data: photos } = await db.from('container_photos').select('*').in('container_id', ids).order('sort_order', { ascending: true });
    const photoMap = {};
    (photos || []).forEach(p => { if (!photoMap[p.container_id]) photoMap[p.container_id]=[]; photoMap[p.container_id].push(p); });
...
function getFilteredContainers() {
  ...
  let c = [...allContainers];
  if (search)   c = c.filter(x => x.code?.toLowerCase().includes(search)||x.description?.toLowerCase().includes(search)||x.storage_bin_types?.name?.toLowerCase().includes(search)||x.location?.toLowerCase().includes(search));
  ...
  list.innerHTML = containers.map(c => {
    ...
    return `<div class="container-card" onclick="openContainerDetail('${c.id}')">
      ...
      <div class="container-body">
        ...
        <div class="container-type-label">${escapeHtml(c.storage_bin_types?.name||'—')}</div>
      ...
      <div class="container-right">
        ...
        <div class="container-actions">
          <button class="btn btn-ghost" onclick="event.stopPropagation();editContainer('${c.id}')" title="Edit"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="14" height="14"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
          <button class="btn btn-danger" onclick="event.stopPropagation();deleteContainer('${c.id}')" title="Delete"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="14" height="14"><polyline points="3 6 5 6 21 6"/><path stroke-linecap="round" stroke-linejoin="round" d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg></button>
        </div>
      </div>
    </div>`;
  }).join('');
...
function openContainerDetail(id) {
  ...
  document.getElementById('detail-link').innerHTML       = c.product_link?`<a href="${c.product_link}" target="_blank">${escapeHtml(c.product_link)}</a>`:'—';
  ...
  gallery.innerHTML = photos.length===0
    ...
    : photos.map(p=>`<div class="detail-photo-item"><img src="${p.url}" alt="${p.label}" onclick="window.open('${p.url}','_blank')"><span class="photo-label-badge">${p.label}</span></div>`).join('');
  document.getElementById('detail-edit-btn').onclick = () => { closeModal('container-detail-modal'); editContainer(id); };
...
function handlePhotoSelect(input) {
  Array.from(input.files).forEach(file => {
    ...
    reader.onload = e => {
      ...
      item.id = `ppho-${id}`;
      item.innerHTML = `<img src="${e.target.result}" alt="preview"><select class="photo-label-select" onchange="updatePendingLabel('${id}',this.value)"><option value="exterior">Exterior</option><option value="interior">Interior</option><option value="detail">Detail</option></select><button class="photo-delete-btn" onclick="removePendingPhoto('${id}')">✕</button>`;
...
function updatePendingLabel(id,label) { const p=window._pendingPhotos.find(x=>x.id===id); if(p) p.label=label; }
function removePendingPhoto(id) { window._pendingPhotos=window._pendingPhotos.filter(x=>x.id!==id); document.getElementById(`ppho-${id}`)?.remove(); }
...
async function uploadPendingPhotos(containerId) {
  for (let i=0; i<window._pendingPhotos.length; i++) {
    ...
    await db.from('container_photos').insert({container_id:containerId, url:urlData.publicUrl, label:pending.label, sort_order:i});
  }
}
...
async function saveContainer() {
  ...
  btn.disabled=false; btn.textContent = id?'Save Changes':'Add Container';
...
acync function deleteBinType(id) {
...
function openInsuranceReport() {
  ...
  document.getElementById('report-date').textContent  = 'As of '+new Date().toLocaleDateString('en-US',{month:'numeric',day:'numeric',year:'numeric'});
  const valued = allContainers.filter(c=>c.estimated_value>0).sort((a,b)=>b.estimated_value-a.estimated_value);
  document.getElementById('insurance-list').innerHTML = valued.length===0
    ...
    : valued.map(c=>`<div class="insurance-row"><div><div class="insurance-code">${escapeHtml(c.code||'')}</div><div class="insurance-detail">${escapeHtml(c.storage_bin_types?.name||'')}${c.location?' · '+escapeHtml(c.location):''}</div>${c.description?`<div class="insurance-desc">${escapeHtml(c.description.substring(0,60))}${c.description.length>60?'…':''}</div>`:''}</div><div class="insurance-value">${formatCurrency(c.estimated_value)}</div></div>`).join('');
  openModal('insurance-modal');
}

function insertLocationHint(hint) {
  const f = document.getElementById('container-location');
  f.value=hint; f.focus();
}
